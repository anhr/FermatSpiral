						...
						input: {
							...
							params: {
								...
								u32: {
									...
									count: settings.count,//points count меньше общего количества точек для того что бы точки, расположенные на краю спирали имели ребра направленные к этим лишним точкам.
...
//										compareArrays(pointsDebug, points);
							...
							//edges
							{
								...
								count:
									1 +//length
									edgesCount * edgesRowlength,
								phase: 1,
								out: out => {

									const edges = [], uInt32Array = new Uint32Array(out), length = uInt32Array[0];
									
									console.log('edges: length = ' + length);
									
									for (var i = 0, j = 1; i < length; i++, j += edgesRowlength) {

										edges.push({
											i: uInt32Array[j],//индекс ребра
											verticesIndices : [uInt32Array[j + 1], uInt32Array[j + 2]],//индексы вершин ребра
											debug : [uInt32Array[j + 3], uInt32Array[j + 4]],
										});
									...
									edges.forEach(item => {
										let log = '';
										Object.keys(item).forEach(key => log += ' ' + key + ': ' + item[key]);
										console.log(log);
									});
			...
			function createEdgesAndFaces(points) {
				...
				//edges
				//edgesItem is array: массив с индексами вершин ребра
				//У массива с индексами вершин ребра есть свойство i: индекс ребра
